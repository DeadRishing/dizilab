...
class Shows{
    ...
    {
        if ($data = $this->CI->show_model->get_show($permalink)) {
            $det1 = $data[0];
            return $det1;
        }
        return false;
    ...
    {
        if ($data = $this->CI->show_model->get_episode($permalink,$season,$episode)) {
            $det2 = $data[0];
            if(!existCookie('view_ep', $det2['id'])){
                if($this->CI->show_model->viewed_ep($det2['id'])){
    ...
    function get_episodet($category_id,$seasons= FALSE,$sub_category='',$sub_category_id=0){
        ...
        if($videos = $this->CI->show_model->sezonlarr($category_id)){
